...
const TopicManager = () => {
  const { data: rawTopics, loading: loadingTopics, setData: setTopics } = useFetch(API.TOPICS.LIST);
  const { data: rawModules, loading: loadingModules } = useFetch(API.MODULES.LIST);
  ...
  const initialForm = { module: "", name: "" };
  const [formData, setFormData] = useState(initialForm);

  const [deleteModal, setDeleteModal] = useState({ show: false, id: null });
  const [isDeleting, setIsDeleting] = useState(false);

  // --- SAFETY CHECK & FLATTENING ---
  const safeModulesData = Array.isArray(rawModules) ? rawModules : (rawModules?.data || rawModules?.results || []);
  const safeTopicsData = Array.isArray(rawTopics) ? rawTopics : (rawTopics?.data || rawTopics?.results || []);

  const flattenedTopics = safeTopicsData.flatMap(mod => 
  ...
  // --- HANDLERS ---
  const handleEdit = (topic, moduleId) => {
    setFormData({
      module: moduleId,
      name: topic.name
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      // --- UPDATE (PATCH) ---
      result = await submitData(API.TOPICS.DETAIL(editingId), payload, true, 'PATCH');
      ...
      if (result.success) {
        setTopics((prev) => {
            ...
            return list.map(mod => {
                // 1. Is this the TARGET module? (Where we want the topic to be)
                if (mod.module_id === payload.module) {
                    const exists = mod.topics.find(t => t.id === editingId);
                    
                    if (exists) {
                        // If it's already here, just update the name
                        return { 
                            ...mod, 
                            topics: mod.topics.map(t => t.id === editingId ? { ...t, name: payload.name } : t) 
                        };
                    } else {
                        // It moved here from another module. Add it.
                        return { 
                            ...mod, 
                            topics: [...mod.topics, { id: editingId, name: payload.name }] 
                        };
                    }
                }

                // 2. Is this the OLD module? (Where it shouldn't be anymore)
                // Filter it out just in case it was here before
                return {
                    ...mod,
                    topics: mod.topics.filter(t => t.id !== editingId)
                };
            });
        });
      ...
      if (result.success) {
        const newTopic = { id: result.data.id, name: payload.name };
  ...
  const confirmDelete = async () => {
    ...
    if (result.success) {
      setTopics((prev) => {
          ...
          return list.map(mod => ({
              ...mod,
              topics: mod.topics.filter(t => t.id !== id)
          }));
      });
          ...
          <table className="min-w-full">
            <thead className="bg-gray-800 text-white">
              <tr>
                ...
                <th className="py-3 px-6 text-left w-1/3">Module Name</th>
                <th className="py-3 px-6 text-left">Topic Name</th>
                <th className="py-3 px-6 text-center w-40">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {flattenedTopics.length > 0 ? flattenedTopics.map((item, index) => (
                <tr key={item.id || index} className="hover:bg-blue-50 transition">
                  ...
                  <td className="py-4 px-6 text-center flex justify-center gap-3">
                    <button onClick={() => handleEdit(item, item.module_id)} className="bg-amber-500 text-white p-2 rounded hover:bg-amber-600 shadow-sm transition cursor-pointer">
                ...
                <tr>
                   <td colSpan="4" className="py-10 text-center text-gray-400">No topics found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-md p-6 rounded-2xl shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Select Module</label>
                <select 
                  name="module" 
              ...
              <div className="flex gap-3 mt-2">
                ...
                >
                  {submitting ? <Spinner /> : (editingId ? "Update" : "Save")}
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-sm p-6 rounded-2xl shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-center text-gray-900 mb-2">Delete Topic?</h3>
